	...
	public void cutter(File tifffile, String result) throws Exception
	{
		...
		for (int k = 0; k < images.size(); k++)
		{
			...
			if (p.getMidmaximun() >= (int) (0.008 * img.getWidth()) && p.getLine() == 0)
			{
				...
				System.out.println("page " + (k + 1) + " is cropped. and midlenth = " + p.getMidmaximun() + " and .007 = "
						+ (int) (0.007 * img.getWidth()) + " and line = " + p.getLine());
			}
			else
			{
				output.add(img.getSubimage(p.getL2(), 0, p.getR1() - p.getL2(), img.getHeight()));
				System.out.println("page " + (k + 1) + " is not cropped.and midlenth = " + p.getMidmaximun() + " and .007 = "
						+ (int) (0.007 * img.getWidth()) + " and line = " + p.getLine());
			}
		}
		tc.tiff_Maker(output, result);
		// tc.tiff_Maker(input, "C:/Users/ATUL/Desktop/Page-layout/testing/output/" +
		// FilenameUtils.removeExtension(tifffile.getName())
		// + "_deskew.tif");
		// tc.tiff_Maker(images, "C:/Users/ATUL/Desktop/Page-layout/testing/output/" +
		// FilenameUtils.removeExtension(tifffile.getName())
		// + "_deskew.tif");
	}
